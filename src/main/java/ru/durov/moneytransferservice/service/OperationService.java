...
public class OperationService {
    ...
    public Operation getOperationFromOperationDTO(OperationDTO operationDTO) {
        ...
        return Operation.builder()
                ...
                .commission(commissionService.getCommissionFromValue(amountDTO.getValue()))
                .build();

    }

    public boolean confirmOperation(ConfirmOperationDTO confirmOperationDTO) {
        UUID operationUUID = UUID.fromString(confirmOperationDTO.getOperationId());
        Operation operation = findOperationById(operationUUID).orElseThrow(CardNotFoundException::new);
        if(operation.getVerificationCode().equals(confirmOperationDTO.getCode())){
            return true;
        } else {
            throw new ConfirmationError("Введен неверный подтверждающий код.");
        }
    ...
    public String doTransaction(String operationId) {
        ...
        BigDecimal total = operation.getValue().add(operation.getCommission());
        Card cardFrom = operation.getCardFrom();
        Card cardTo = operation.getCardTo();
        if(cardFrom.getBankAccount().getValue().compareTo(total) < 0) {
            throw new InputDataError("Недостаточно средств.");
        }
        cardFrom.getBankAccount().setValue(cardFrom.getBankAccount().getValue().subtract(total));
        cardTo.getBankAccount().setValue(cardTo.getBankAccount().getValue().add(operation.getValue()));
